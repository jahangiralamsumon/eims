...
class EmployeesController extends Controller
{
	...
	public function actionCreate()
	{
		...
		$row =Employees::model()->find($criteria);
		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
		$model->id=$row->id+1;
		$model->emp_number='E'.$model->id;
		if(isset($_POST['Employees']))
		{
			...
			if($valid){
			if($file=CUploadedFile::getInstance($model,'photo_file_name'))
			{
					
				$path=Yii::app()->basePath.'/../uploadedfiles/employees';
				if (!is_dir($path))
				{
					mkdir($path,0777);
				}
					
				$path=Yii::app()->basePath.'/../uploadedfiles/employees/'.$model->id;
				if (!is_dir($path))
				{
					mkdir($path,0777);
				}					
