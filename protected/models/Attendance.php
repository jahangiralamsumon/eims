 ...
 * @property string $attendance_card_id
...
class Attendance extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('status,attendance_card_id,date', 'required'),
			array('status,student_id', 'numerical', 'integerOnly'=>true),
			array('in_time,out_time','safe'),	
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('attendance_id, status, student_id, date', 'safe', 'on'=>'search'),
		);
	}
	...
	public function attributeLabels()
	{
		return array(
			...
			'status' => '0 undefined , 1 present , 2  absent',
			'student_id' => 'Student',
			'date' => 'Date',
		);
	}
